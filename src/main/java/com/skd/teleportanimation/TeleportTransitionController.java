...
import com.skd.teleportanimation.BobbyCompat;
import com.skd.teleportanimation.DimensionIds;
import com.skd.teleportanimation.DistantHorizonsCompat;
import com.skd.teleportanimation.TeleportClient;
import com.skd.teleportanimation.TeleportConfig;
import com.skd.teleportanimation.IrisCompat;
import com.skd.teleportanimation.SodiumCompat;
import com.skd.teleportanimation.TeleportDestinationParser;
import com.skd.teleportanimation.TeleportSounds;
import com.skd.teleportanimation.VoxyCompat;
...
public final class TeleportTransitionController {
    ...
    private static double tryGetSkyFacingSurfaceY(Minecraft client, Vec3 feet) {
        ...
        if (surfaceY <= client.level.getMinBuildHeight()) {
            return Double.NaN;
        }
        return surfaceY;
    }
    ...
    private static CameraFrame resolveLeawindReleaseCameraFrame(String className, float pitch, float yaw) {
        try {
            ...
            if (!(rawCamera instanceof Camera) || (camera = (Camera)rawCamera).getPosition() == null) {
                return null;
            }
            return new CameraFrame(camera.getPosition(), camera.getYRot(), camera.getXRot());
        }
    ...
    private static void captureEnterBodyTarget(LocalPlayer player) {
        ...
        if (TeleportTransitionController.shouldRespectPreviousCameraPosition()) {
            ...
            if (camera != null && liveCameraPos != null) {
                enterBodyTargetYaw = camera.getYRot();
                enterBodyTargetPitch = camera.getXRot();
                return;
            }
    ...
    private static Vec3 getCurrentCameraPosition(Vec3 playerEye) {
        Camera camera = TeleportTransitionController.getCurrentCamera();
        if (camera == null || camera.getPosition() == null || playerEye == null) {
            return null;
        }
    ...
    private static Vec3 getInitialTransitionCameraPosition(Minecraft client, Vec3 fallback) {
        ...
        return camera == null || camera.getPosition() == null ? fallback : camera.getPosition();
    }
    ...
    private static float getInitialTransitionYaw(Minecraft client, LocalPlayer player) {
        ...
        Camera camera = client.gameRenderer.getMainCamera();
        return camera == null ? player.getYRot() : camera.getYRot();
    }
    ...
    private static float getInitialTransitionPitch(Minecraft client, LocalPlayer player) {
        ...
        Camera camera = client.gameRenderer.getMainCamera();
        return camera == null ? player.getXRot() : camera.getXRot();
    }
